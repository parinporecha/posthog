...
def _apply_duration_filtering(queryset, thresholds: QueryPercentileThresholds | None):
    ...
    Note:
        - max_threshold_seconds of 100+ (p100) means no upper limit, only lower limit
        - Cohorts without duration data (NULL) are included when max_threshold is p100
    """
    if not thresholds:
        return queryset
    ...
    # Special case: if max threshold indicates p100 (no upper limit), only apply lower bound
    # and include cohorts without duration data (NULL values)
    if thresholds.max_threshold_seconds >= 100.0:
...
async def get_query_percentile_thresholds_activity(
    ...
    def get_percentile_thresholds():
        ...
        WHERE lc_feature IN 'behavioral_cohorts'
    ...
    def get_selected_cohort_ids():
        ...
        if inputs.team_ids:
            ...
            if valid_team_ids:
                ...
                if thresholds and (
                    ...
                    original_count = team_cohort_queryset.count()
                    team_cohort_queryset = _apply_duration_filtering(team_cohort_queryset, thresholds)
                    filtered_count = team_cohort_queryset.count()
                    LOGGER.info(f"Duration filtering for team cohorts: {original_count} -> {filtered_count}")
        ...
        if inputs.global_percentage and inputs.global_percentage > 0.0 and inputs.global_percentage <= 1.0:
            ...
            if thresholds and (
                ...
                original_count = other_teams_queryset.count()
                other_teams_queryset = _apply_duration_filtering(other_teams_queryset, thresholds)
                filtered_count = other_teams_queryset.count()
                LOGGER.info(f"Duration filtering for global percentage cohorts: {original_count} -> {filtered_count}")
