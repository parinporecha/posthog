...
describe('createPrepareEventStep', () => {
    ...
    it('should process AI events through processAiEvent', async () => {
        ...
        expect(result.type).toBe(PipelineResultType.OK)
        if (result.type === PipelineResultType.OK) {
            expect(result.value.preparedEvent.properties).toEqual({ enriched: true })
        }
    })

    it('should swallow processAiEvent errors and use original event', async () => {
        const aiEvent = createTestPluginEvent({ event: '$ai_generation', properties: { original: true } })
        ...
        expect(result.type).toBe(PipelineResultType.OK)
        if (result.type === PipelineResultType.OK) {
            expect(result.value.preparedEvent.properties).toEqual({ original: true })
        }
    })
