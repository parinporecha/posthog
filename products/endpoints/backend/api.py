...
class EndpointViewSet(TeamAndOrgViewSetMixin, PydanticModelMixin, viewsets.ModelViewSet):
    ...
    def _build_breakdown_filter_condition(
        ...
        - 0: single breakdown — use has() on the full array (legacy behavior)
        - 1+: multiple breakdowns — use equality on breakdown_value[N]
        """
        if array_index > 0:
            # Multiple breakdowns: breakdown_value is Array(String), access by index
    ...
    def _variables_to_filters(
        ...
        breakdown_info: tuple[str, str] | None = None,
        ...
        Args:
            ...
            breakdown_info: Single (property_name, property_type) — legacy, for backwards compat
            breakdown_infos: List of (property_name, property_type) — for multiple breakdowns
        """
        ...
        # Build properties filter for breakdowns
        all_infos = breakdown_infos or ([breakdown_info] if breakdown_info else [])
        properties: list[dict] | None = None
        for prop_name, prop_type in all_infos:
