...
NON_REAGGREGATABLE_FUNCTIONS = frozenset({"avg", "uniq", "uniqExact", "avgWeighted"})
...
def analyze_variables_for_materialization(
    ...
    # TODO: Does it have to be a range, or should a single >= be applicable too?
    ...
    if has_range_vars:
        try:
            ...
            if isinstance(ast_node, ast.SelectQuery) and ast_node.select:
