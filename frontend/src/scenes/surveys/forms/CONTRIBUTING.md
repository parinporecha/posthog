...
- **Document:** the TipTap document, stored as `survey.form_data.content`, the source of truth for Form data
...
Your new form question type will now be accessible from the plus button / slash command menus and get stored properly in both `survey.form_data` and `survey.questions`.
